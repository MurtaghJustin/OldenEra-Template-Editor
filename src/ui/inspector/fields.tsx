...
export function ReferenceListField({ label, values, options, onChange, onAddNew }:
  { label: string; values: string[]; options: string[]; onChange: (next: string[]) => void; onAddNew?: () => void; }) {
  const id = useId(); const listId = id + "-list";
    ...
    <div style={{ display: "grid", gap: 4, marginBottom: 8, minWidth: 0 }}>
      ...
      {values.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
          {values.map((v) => (
            <span key={v} title={v} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, maxWidth: "100%",
      ...
      <div style={{ display: "flex", gap: 4, minWidth: 0 }}>
        ...
        <button type="button" disabled title="Creating a new group — coming soon" onClick={onAddNew} style={{ flexShrink: 0 }}>+ New</button>
      </div>
    </div>
  );
}
